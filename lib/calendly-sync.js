// mikes-crm-api/lib/calendly-sync.js
...
async function syncBookings() {
  ...
  try {
    ...
    const enquiries = await listEnquiries();   // ✅ was missing await
    const existing  = await getBookings();     // ✅ was missing await
    ...
    for (const event of events) {
      ...
      for (const invitee of invitees) {
        ...
        await saveBooking({          // ✅ was missing await
        ...
        if (match && status === 'confirmed') {
          await updateStatus(match.id, 'booked');   // ✅ was missing await
          await saveReply({                          // ✅ was missing await
            enquiry_id: match.id,
            from_name:  'Calendly',
            from_type:  'auto',
        ...
        if (match && status === 'cancelled') {
          await cancelBooking(email);               // ✅ was missing await
          await updateStatus(match.id, 'replied');  // ✅ was missing await
          await saveReply({                          // ✅ was missing await
            enquiry_id: match.id,
            from_name:  'Calendly',
            from_type:  'auto',
